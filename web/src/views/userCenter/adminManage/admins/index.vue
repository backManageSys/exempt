<template>
  <div class="app-container">
    ...
    <el-table
      ...
      <el-table-column prop="operatorName" label="操作上级" align="center"/>
      <el-table-column prop="addTimep" label="添加时间" align="center" min-width="100%"/>
      <el-table-column prop="status" label="状态" align="center"/>
      <el-table-column label="操作" fixed="right" align="center" >
        <template scope="scope">
          <el-button size="small" @click="openDialog(scope.$index,scope.row)">
            修改
          </el-button>
        </template>
      </el-table-column>
    ...
    <!--点击 修改 按钮以后弹出的窗口-->
    <el-dialog :visible.sync="dialogFormVisible" title="修改管理员信息" >
      <el-form ref="form" :model="newRow.user" :rules="addRules" label-width="13%">
        <el-form-item label="用户名" prop="username">
          <el-input v-model="newRow.user.username" disabled="disabled" placeholder="请输入用户名" style="width:90%;"/>
        </el-form-item>
        <el-form-item label="密码" prop="password">
          <el-input v-model="newRow.user.password" type="password" placeholder="请输入密码" style="width:90%;"/>
        </el-form-item>
  ...
  methods: {
    ...
    getStaff() {
      adminsGet().then(response => {
        console.log(response, '/admins 查看全部管理员  赋值给teams')
    ...
    getTeams() {
      teamsGet().then(response => {
        console.log(response, '/company/teams 所有团队 赋值给teams1')
        if (response.code != 200) {
          this.$message({
            message: response.data.description,
            type: 'warning'
    ...
    getPosts() {
      postGet().then(response => {
        console.log(response, '/company/post/list 查看全部岗位 赋值给posts')
        if (response.code != 200) {
          this.$message({
            message: response.data.description,
            type: 'warning'
